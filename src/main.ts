/**
 * AMBIENT PRESENCE v9
 * - Friend orbs: rich round glowing blobs with bright white core (like reference image)
 * - YOU orb: full 3D volumetric raymarch (restored)
 * - Labels/names centered ON the orb center (profile pic placeholder)
 * - FBO scale tuned for 60fps
 */
...
// YOU uses same 2D FBM shader — white/silver so it reads as "you"
const YOU_PAL = {a:[1.0,1.0,1.0], b:[0.75,0.88,1.0], c:[0.9,0.95,1.0]};

// ─── Data model ────────────────────────────────────────────────────────────
interface Friend {
  id:number; name:string;
  density:number;  // 0=far/wispy  1=near/solid
  bearing:number;  // 0=top 90=right 180=bottom 270=left
...
// ─── Geometry ──────────────────────────────────────────────────────────────
// Friend orbs sit at ORB_REACH fraction of the way to the screen edge
...
const LBL_W=80, LBL_H=36, LBL_PAD=6, SPREAD_ITERS=80;
interface LblPos{x:number;y:number;}

function resolveCollisions(positions:LblPos[]):void {
  for(let iter=0;iter<SPREAD_ITERS;iter++){
    ...
    for(let i=0;i<positions.length;i++){
      for(let j=i+1;j<positions.length;j++){
        ...
        const ox=(LBL_W+LBL_PAD)-Math.abs(dx);
        const oy=(LBL_H+LBL_PAD)-Math.abs(dy);
        if(ox>0&&oy>0){
          if(ox<oy*1.4){const sx=dx>=0?1:-1;a.x-=sx*ox*0.5;b.x+=sx*ox*0.5;}
          else          {const sy=dy>=0?1:-1;a.y-=sy*oy*0.5;b.y+=sy*oy*0.5;}
          moved=true;
        }
      }
    }
  ...
  for(const p of positions){
    p.x=Math.max(LBL_W/2+mg, Math.min(window.innerWidth -LBL_W/2-mg, p.x));
    p.y=Math.max(LBL_H/2+mg, Math.min(window.innerHeight-LBL_H/2-mg, p.y));
  }
}
...
const FU={time:u(FP,'uTime'),seed:u(FP,'uSeed'),dens:u(FP,'uDensity'),rad:u(FP,'uRadius'),
          cen:u(FP,'uCenter'),res:u(FP,'uRes'),ca:u(FP,'uCA'),cb:u(FP,'uCB'),cc:u(FP,'uCC')};
...
const FBO_SCALE=MOB?0.38:0.48; // fast = 60fps; soft upscale suits cloud aesthetic
const FRD_R=0.26; // friend orb radius in isotropic UV units
...
// ─── Draw any orb (friend or YOU) ────────────────────────────────────────
function drawFriend(t:number,uv:[number,number],density:number,seed:number,pal:{a:number[];b:number[];c:number[]},radius=FRD_R){
  gl.useProgram(FP);bindQuad(FP);
  gl.uniform1f(FU.time,t);gl.uniform1f(FU.seed,seed);gl.uniform1f(FU.dens,density);
  gl.uniform1f(FU.rad,radius);gl.uniform2f(FU.cen,uv[0],uv[1]);
  gl.uniform2f(FU.res,FW,FH);
  gl.uniform3fv(FU.ca,pal.a);gl.uniform3fv(FU.cb,pal.b);gl.uniform3fv(FU.cc,pal.c);
...
function frame(ts:number){
  ...
  // YOU — same orb shader, white/silver palette, bigger radius, unique seed=0
  drawFriend(t,[0.5,0.5],0.95,0.0,YOU_PAL,0.34);

  // Friends
  friends.forEach((f,i)=>{
    if(!f.active)return;
    const {x,y}=orbPx(f.bearing);
    drawFriend(t,cssToGLuv(x,y),f.density,(i+1)*7.391,PAL[f.colorIdx]);
  });
...
// Will be replaced by profile picture in future
document.body.appendChild(el(
  'position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);'+
  'color:rgba(255,255,255,0.85);font:bold 12px/1.3 system-ui,sans-serif;letter-spacing:0.5px;'+
  'text-shadow:0 0 12px #fff,0 0 24px rgba(200,220,255,0.8),0 0 2px rgba(0,0,0,1);'+
...
// ─── Friend labels — centered ON the orb ─────────────────────────────────
// Labels sit exactly at the orb's GL-center converted to CSS px.
// This is where the profile picture will be placed in future.
...
panel.addEventListener('input',(e:Event)=>{
  ...
  }else{friends[id].bearing=+inp.value;}
  buildLabels();
});
...
hint.addEventListener('click',togglePanel);
